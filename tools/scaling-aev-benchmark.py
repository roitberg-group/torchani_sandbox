...
def checkgpu(device=None):
    ...
    print(
        "   GPU Memory Used (nvidia-smi): {:7.1f}MB / {:.1f}MB ({})".format(
            info.used / 1024 / 1024, info.total / 1024 / 1024, name
        )
    )
...
def benchmark(
    speciesPositions, aev_comp, runbackward=False, mol_info=None, verbose=True
):
    global runcounter
...
def check_speedup_error(aev, aev_ref, force_cuaev, force_ref, speed, speed_ref):
    if (
        (speed_ref is not None)
        and (speed is not None)
        and (aev is not None)
        and (aev_ref is not None)
    ):
        # aev error
...
def run(file, nnp_ref, nnp_cuaev, runbackward, maxatoms=10000):
    ...
    print(
        f"File: {file}, Molecule size: {molsize} / {realmolsize}, Species: {spelist}\n"
    )
    ...
    if not args.nsight:
        ...
        aev_ref, delta_ref, force_ref = benchmark(
            speciesPositions, nnp_ref.aev_computer, runbackward, mol_info
        )
        print()
    else:
        delta_ref = None
    ...
    # warm up
    _, _, _ = benchmark(
        speciesPositions, nnp_cuaev.aev_computer, runbackward, mol_info, verbose=False
    )
    # run
    aev, delta, force_cuaev = benchmark(
        speciesPositions, nnp_cuaev.aev_computer, runbackward, mol_info
    )
...
def plot(maxatoms, aev_fd, cuaev_fd, aev_fdbd, cuaev_fdbd):
    ...
    if usetex:
        ...
        rc(
            ...
            **{
                "size": 16,
                "family": "serif",
                "sans-serif": ["Lucid", "Arial", "Helvetica"],
    ...
    plt.title(
        f"Benchmark of cuaev (cuda extension) and pyaev (torch operators)\n"
        f"on {getGpuName()}\n"
        f"{energy_label}",
        fontsize=16,
    )
...
def run_for_plot(file, maxatoms, nnp_ref, nnp_cuaev):
    ...
    for maxatom in maxatoms:
        cuaev_t, aev_t = run(
            file, nnp_ref, nnp_cuaev, runbackward=False, maxatoms=maxatom
        )
    ...
    for maxatom in maxatoms:
        cuaev_t, aev_t = run(
            file, nnp_ref, nnp_cuaev, runbackward=True, maxatoms=maxatom
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--nsight", action="store_true", help="use nsight profile"
    )
    parser.add_argument(
        "-p", "--plot", action="store_true", help="plot benchmark result"
    )
    parser.add_argument(
        "-e", "--run_energy", action="store_true", help="Run NN to predict energy"
    )
    parser.add_argument(
        "--single_nn",
        action="store_true",
    ...
    parser.add_argument(
        "--infer_model", action="store_true", help="Energy: Use infer model"
    )
    parser.add_argument(
        "--mnp",
        action="store_true",
    ...
    parser.add_argument("-n", "--N", help="Number of Repeat", default=200, type=int)
    parser.add_argument(
        "--use-cell-list",
