...
def save_xyz_geometries(species, coordinates):
    ...
    mu.tensor_to_xyz(xyz_path, (species, coordinates))
...
if __name__ == '__main__':
    ...
    if plot:
        ...
        exit()
    cutoff = 8.5
    start_distance = 0.2  # or 0.1?
    end_distance = 8.5
    orders = [4]
    models = {'ANI-1x': ANI1x, 'ANI-2x': ANI2x, 'ANI-1ccx': ANI1ccx, 'ANI-D': ANID}
    energy_calculators = {}
    if pure_d3:
        for order in orders:
    ...
    for j, (molecule, maker) in enumerate(makers.items()):
        ...
        for d in displacements:
            ...
            for name, calc in energy_calculators.items():
                energies[name].append(calc((species, coordinates.float())).energies.item())
