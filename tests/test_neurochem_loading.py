...
class TestLoading(TestCase):
    ...
    def testANI1x(self):
        model = ANI1x()
        model_nc = load_builtin_from_name("ani1x")
        self.assertEqual(model_nc.state_dict(), model.state_dict())

    def testANI1xSingle(self):
        for j in range(8):
            model = ANI1x(model_index=j)
            model_nc = load_builtin_from_name("ani1x", model_index=j)
            self.assertEqual(model_nc.state_dict(), model.state_dict())

    def testANI2x(self):
        model = ANI2x()
        model_nc = load_builtin_from_name("ani2x")
        self.assertEqual(model_nc.state_dict(), model.state_dict())

    def testANI2xSingle(self):
        for j in range(8):
            model = ANI2x(model_index=j)
            model_nc = load_builtin_from_name("ani2x", model_index=j)
            self.assertEqual(model_nc.state_dict(), model.state_dict())

    def testANI1ccx(self):
        model = ANI1ccx()
        model_nc = load_builtin_from_name("ani1ccx")
        self.assertEqual(model_nc.state_dict(), model.state_dict())
