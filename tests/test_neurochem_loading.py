...
class TestModelLoader(ANITest):
    ...
    def testANI1x(self):
        ...
        model_nc = self._setup(torchani.neurochem.load_builtin_from_name("ani1x"))
    ...
    def testANI1xSingle(self):
        for j in range(8):
            ...
            model_nc = self._setup(
                torchani.neurochem.load_builtin_from_name("ani1x", model_index=j)
            )
            self.assertEqual(model_nc.state_dict(), model.state_dict())

    def testANI2x(self):
        model = self._setup(ANI2x())
        model_nc = self._setup(torchani.neurochem.load_builtin_from_name("ani2x"))
        self.assertEqual(model_nc.state_dict(), model.state_dict())
    ...
    def testANI2xSingle(self):
        for j in range(8):
            ...
            model_nc = self._setup(
                torchani.neurochem.load_builtin_from_name("ani2x", model_index=j)
            )
            self.assertEqual(model_nc.state_dict(), model.state_dict())

    def testANI1ccx(self):
        model = self._setup(ANI1ccx())
        model_nc = self._setup(torchani.neurochem.load_builtin_from_name("ani1ccx"))
        self.assertEqual(model_nc.state_dict(), model.state_dict())
...
class TestAEVLoader(ANITest):
    ...
    def testEqualNeurochem1x(self):
        aev_1x_nc, _ = torchani.neurochem.load_aev_computer_and_symbols(const_file_1x)
        aev_1x = AEVComputer.like_1x()
        self._compare_constants(aev_1x_nc, aev_1x)

    def testEqualNeurochem2x(self):
        aev_2x_nc, _ = torchani.neurochem.load_aev_computer_and_symbols(const_file_2x)
        aev_2x = AEVComputer.like_2x()
        self._compare_constants(aev_2x_nc, aev_2x)

    def testEqualNeurochem1ccx(self):
        aev_1ccx_nc, _ = torchani.neurochem.load_aev_computer_and_symbols(
            const_file_1ccx
        )
