...
class TestDispersion(ANITest):
    ...
    def testConstructor(self):
        disp = self._setup(
            TwoBodyDispersionD3.from_functional(symbols=SYMBOLS_1X, functional="B973c")
        )
        self.assertTrue(disp._s6 == torch.tensor(1.0, device=self.device))

    def testMethaneCoordinationNums(self):
        disp = self._setup(
            TwoBodyDispersionD3.from_functional(symbols=SYMBOLS_1X, functional="wB97X")
        )
    ...
    def testForce(self):
        model = self._setup(
            StandaloneTwoBodyDispersionD3(symbols=SYMBOLS_1X, functional="wB97X")
        )
