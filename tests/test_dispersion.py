...
class TestALAtomic(TestCase):
    ...
    def testMethaneCoordinationNums(self):
        ...
        _, _, atom_index12, distances =\
                self.aev_computer((species_idx, self.coordinates))
        energy = torch.tensor([0.0], dtype=self.coordinates.dtype, device=self.device)
        disp = DispersionD3().to(self.device)
        torch.set_printoptions(precision=10)
        distances = units.angstrom2bohr(distances)
        coordnums  = disp._get_coordnums(self.coordinates.shape[1], self.species.flatten()[atom_index12], atom_index12, distances)
        # coordination numbers taken directly from DFTD3 Grimme et. al. code
        self.assertTrue(torch.isclose(coordnums.cpu(), torch.tensor([1.0052222, 1.0052222, 1.0052222, 1.0052222, 3.999873048]).double()).all())

        #print(disp((self.species, energy), atom_index12, distances))
    ...
    def testPrecomputedC6(self):
        ...
        expect_diags = {
        'H' : torch.tensor([3.0267000198, 7.5915999413, -1.0000000000, -1.0000000000, -1.0000000000]), 
        'C' : torch.tensor([49.1129989624, 43.2452011108, 29.3602008820, 25.7808990479, 18.2066993713]), 
        'O' : torch.tensor([25.2684993744, 22.1240997314, 19.6767997742, 15.5817003250, -1.0000000000]), 
        'N' : torch.tensor([15.5059003830, 12.8161001205, 10.3708000183,  -1.0000000000, -1.0000000000])}
        for k in diags.keys():
            self.assertTrue(torch.isclose(diags[k].cpu(), expect_diags[k]).all())

    def testAllCarbonConstants(self):
        c6_constants, _, _ = constants.get_c6_constants()
        expect_c6_carbon = torch.tensor([[49.1130, 46.0681, 37.8419, 35.4129, 29.2830],
        ...
        self.assertTrue(torch.isclose(expect_c6_carbon, c6_constants[6, 6]).all())
    ...
    def testMethaneC6(self):
        ...
        coordnums  = disp._get_coordnums(self.coordinates.shape[1], species12, atom_index12, distances)
        order6_coeffs = disp._interpolate_order6_coeffs(species12, coordnums, atom_index12)
        # C6 coefficients taken directly from DFTD3 Grimme et. al. code
        expect_order6 = torch.tensor([3.0882003,
            3.0882003, 3.0882003, 7.4632792, 3.0882003, 3.0882003, 7.4632792,
            3.0882003, 7.4632792, 7.4632792]).double()
        self.assertTrue(torch.isclose(order6_coeffs.cpu(),expect_order6).all())
    ...
    def testMethaneEnergy(self):
        ...
        self.assertTrue(torch.isclose(energy.cpu(), torch.tensor([-1.251336]).double()))
    ...
    def testForce(self):
        ...
        # gradient in Bohr)
        expect_grad = torch.tensor([[-0.42656701194940E-05, -0.42656701194940E-05, -0.42656701194940E-05],   
                                    [-0.42656701194940E-05,  0.42656701194940E-05,  0.42656701194940E-05], 
                                    [ 0.42656701194940E-05, -0.42656701194940E-05,  0.42656701194940E-05], 
                                    [ 0.42656701194940E-05,  0.42656701194940E-05, -0.42656701194940E-05], 
                                    [ 0.00000000000000E+00,  0.00000000000000E+00,  0.00000000000000E+00]]).double()
