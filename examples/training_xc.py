...
from torchani.assembler import ExCorrANI

# Explanation of how to train an ANI model with XC energies
...
# performance.
batched_dataset_path = Path('./batched_dataset_xc').resolve()
if not batched_dataset_path.exists():
    torchani.datasets.create_batched_dataset(
        ds,
        dest_path=batched_dataset_path,
        batch_size=2560,  # Probably change depending on ds size
...
kwargs: tp.Any = {"num_workers": 2, "prefetch_factor": 2, "pin_memory": True}
...
# Keep in mind without geometric aev's the default ani networks are too large
# maybe If the featurizer changes maybe add a flag on the featurizer, and on
# the assembler to modify the model
model = ExCorrANI(symbols=("H", "C", "N", "O", "S"), use_geometric_aev=False)

# Example of getting energies and coeffs with the model
# species = torch.tensor([[1, 1, 6, 6]])
# coordinates = torch.randn((1, 4, 3), dtype=torch.float)
# coefficients = torch.torch.randn((1, 4, 45), dtype=torch.float, requires_grad=True)
# energies = model((species, coordinates), coefficients).energies
# coeff_deriv = torch.autograd.grad(energies.sum(), coefficients)[0]
...
def validate(model: BuiltinModel, validation: torch.utils.data.DataLoader) -> float:
    ...
    with torch.no_grad():
        for properties in validation:
            ...
            target_energies = properties['energies'].float()  # Probably change this
            coefficients = properties['coefficients'].float()  # Probably change this
            output = model((species, coordinates), coefficients)
...
for epoch in range(scheduler.last_epoch, max_epochs + 1):
    ...
    for batch in tqdm(
        ...
        target_energies = batch['energies'].float()  # Probably change
        coefficients = batch['coefficients'].float()  # Probably change
