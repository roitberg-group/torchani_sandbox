...
def tile_into_tight_cell(
    species_coordinates: tp.Tuple[Tensor, Tensor],
    repeats: tp.Union[int, tp.Tuple[int, int, int]] = (3, 3, 3),
    noise: tp.Optional[float] = None,
    delta: float = 1.0,
    ...
    if isinstance(repeats, int):
        ...
        repeats_tensor = torch.tensor(repeats, dtype=torch.long, device=device)
        assert (repeats_tensor >= 1).all(), "At least one molecule should be present"
    ...
    x_disp = torch.arange(0, repeats_tensor[0].item(), device=device)
    y_disp = torch.arange(0, repeats_tensor[1].item(), device=device)
    z_disp = torch.arange(0, repeats_tensor[2].item(), device=device)
    ...
    cell_length = box_length * repeats_tensor
    cell = torch.diag(
        torch.tensor(
