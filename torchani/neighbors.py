...
class Neighborlist(torch.nn.Module):
    ...
    def get_diff_vectors(self) -> Tensor:
...
class CellList(Neighborlist):
    ...
    def _fractionalize_coordinates(self, coordinates: Tensor) -> Tensor:
        ...
        assert not torch.isnan(
            fractional_coordinates
        ).any(), "Some fractional coordinates are NaN."
        assert not torch.isinf(
            fractional_coordinates
        ).any(), "Some fractional coordinates are +-Inf."
        assert (
            fractional_coordinates < 1.0
        ).all(), f"Some fractional coordinates are too large {fractional_coordinates[fractional_coordinates >= 1.]}"
        assert (
            fractional_coordinates >= 0.0
        ).all(), f"Some coordinates are too small {fractional_coordinates.masked_select(fractional_coordinates < 0.)}"
