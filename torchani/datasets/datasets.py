...
from torchani.datasets._backends import (
    _H5PY_AVAILABLE,
    _StoreWrapper,
    StoreFactory,
    TemporaryLocation,
...
def _num_conformers(conformers: NumpyConformers) -> int:
    return _dim_size(
        conformers, common_keys={"coordinates", "coord", "forces", "energies"}, dim=0
    )
...
class ANIBatchedDataset(torch.utils.data.Dataset[Conformers]):
    ...
    def __init__(
        ...
        if not batches:
            ...
            self._extractor = self._get_batch_extractor(
                self._batch_paths[0].suffix, file_format
            )
    ...
    def _get_batch_extractor(
        self, suffix: str, file_format: tp.Optional[str] = None
    ) -> tp.Callable[[int], Conformers]:
        if file_format is None:
            try:
                file_format = self._SUFFIXES_AND_FORMATS[suffix]
            except KeyError:
                raise ValueError(
                    f"The file format {file_format} is not one of the"
                    f"supported formats {self._SUFFIXES_AND_FORMATS.values()}"
                )
        if file_format == "hdf5" and not _H5PY_AVAILABLE:
            raise ValueError(
                "File format hdf5 was specified but h5py could not"
                " be found, please install h5py or specify a "
                " different file format"
            )
        return {
            "numpy": self._numpy_extractor,
            "pickle": self._pickle_extractor,
            "hdf5": self._hdf5_extractor,
        }[file_format]
    ...
    def cache(
        self, pin_memory: bool = True, verbose: bool = True
    ) -> "ANIBatchedDataset":
...
class _ANISubdataset(_ANIDatasetBase):
    ...
    def _get_open_store(
        self, stack: ExitStack, mode: str = "r", only_meta: bool = False
    ) -> "_StoreWrapper":
        if mode not in ["r+", "r"]:
    ...
    def present_elements(
        ...
        for group_name in self.keys():
            conformers = self.get_numpy_conformers(
                group_name, properties=element_key, chem_symbols=chem_symbols
            )
        ...
        return sorted(
            {
                s
                for ds in self._datasets.values()
                for s in ds.present_elements(chem_symbols)
            }
        )
        ...
        for j, znumbers in enumerate(elements):
            ...
            for property_ in numpy_conformers:
                if property_ in atomic_properties:
                    numpy_conformers[property_][j] = numpy_conformers[property_][
                        j, idxs
                    ]
            if self.grouping == "by_formula":
