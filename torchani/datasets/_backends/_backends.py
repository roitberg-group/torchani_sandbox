...
_CONCRETE_LOCATIONS = {
    "h5py": _H5TemporaryLocation,
    "zarr": _ZarrTemporaryLocation,
    "pq": _PqTemporaryLocation,
}
...
def StoreFactory(
    ...
    if not _BACKEND_AVAILABLE.get(backend, False):
        raise ValueError(
            f"{backend} could not be found, please install it if supported."
            f" Supported backends are {set(_BACKEND_AVAILABLE.keys())}"
        )
    cls: tp.Type[_StoreWrapper] = tp.cast(
        tp.Type[_StoreWrapper], _CONCRETE_STORES[backend]
    )
