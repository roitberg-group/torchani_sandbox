...
_provided_pkgs = importlib.metadata.metadata(__package__.split(".")[0]).get_all(
    "Provides"
)

cuaev_is_installed = (_provided_pkgs is not None) and (
    "torchani.cuaev" in _provided_pkgs
)
...
class AEVComputer(torch.nn.Module):
    ...
    def __init__(
        ...
        if num_species is None:
            raise ValueError(
                "num_species must be provided to correctly construct an AEVComputer"
            )
    ...
    def _init_cuaev_computer(self):
        assert (
            self.cutoff_fn_type != "others"
        ), "cuaev currently only supports cosine and smooth cutoff functions"
        assert (
            self.cutoff_fn_type != "smooth_modified"
        ), "cuaev currently only supports standard parameters for smooth cutoff function"
    ...
    def _compute_aev(
        ...
        even_closer_indices = (
            (distances <= self.angular_terms.cutoff).nonzero().flatten()
        )
    ...
    def _compute_angular_aev(
        ...
        index = (
            central_atom_index * self.num_species_pairs
            + self.triu_index[species12_[0], species12_[1]]
        )
