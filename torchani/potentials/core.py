...
class BasePairPotential(Potential):
    ...
    def plot(
        ...
        if rmax is None:
            rmax = self.cutoff if self.cutoff != math.inf else 10.0
        r = torch.linspace(rmin, rmax, steps)
        for pair in symbol_pairs:
            ...
            if force:
                ...
                r.detach_()
                energies.detach_()
                ax_force.plot(r, forces, label=f"{pair[0]}-{pair[1]}")
        ...
        if force:
            ax_force.legend()
            ax_force.set_title(f"{self.__class__.__name__}")
            ax_force.set_xlabel(r"Inter atomic distance, ($\AA$)")
            ax_force.set_ylabel(r"Force, ($E_h/\AA$)")
        plt.show()
